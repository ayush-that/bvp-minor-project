...
// -------- agent manifest (kept in sync with backend config.py) --------
...
const AGENTS: AgentMeta[] = [
  {
    ...
    purpose: "Finds live posting URLs on the careers page.",
  },
  {
    id: "extraction",
    label: "extraction",
    provider: "openai",
    model: "gpt-5.4-mini",
    purpose: "Parses each posting into structured JSON.",
  },
  {
    id: "critic",
    label: "critic",
    ...
    purpose: "Audits the extraction against the source and patches drift.",
  },
  {
    id: "matching",
    label: "matching",
    ...
    purpose: "Ranks every posting against the resume, with reasoning.",
  },
];

const PROVIDER_DOT: Record<AgentMeta["provider"], string> = {
  google: "bg-amber-400",
...
function AgentTile({
  ...
  return (
    <div
      className={cn(
        "relative flex flex-col gap-2 border bg-background px-3 py-3 transition-all",
        "rounded-[2px]",
        state === "running" &&
    ...
    >
      <div className="flex items-center gap-2">
        <span className={cn("h-2 w-2 rounded-full", PROVIDER_DOT[meta.provider])} />
        <span
      ...
      <div
        className="text-[11px] leading-tight text-foreground/60 truncate"
        style={{ fontFamily: "JetBrains Mono, monospace" }}
        title={meta.model}
      >
    ...
    <div className="relative">
      ...
      <header className="mb-8">
        ...
        <p className="mt-3 text-sm text-foreground/60 max-w-2xl">
          Four models, one auth surface. Every stage is live — watch the agents
          fire, dispute, and rank in real time.
        </p>
      </header>
          ...
          <div className="p-5 space-y-4 bg-foreground/[0.015]">
            <div>
              ...
              <p className="mt-1.5 text-[11px] text-foreground/50 leading-snug">
                Resumes under 40 words are rejected by the matching agent — add
                education, skills, and at least one project.
              </p>
            </div>
